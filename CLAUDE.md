...
- **E02 教程脚本按新结构重写 + v1.4.1 实测校正**（2026-08-17）：`docs/video-script.md` E02 段重写（+170/−55 行）。新结构：开场痛点→模块一备份（自动监听【重点】/自动定时/手动）→模块二导出（导出当前/导出备份+三格式）→模块三导入（当前浏览器/跨浏览器）→最佳实践（快速下班+跨浏览器搬标签）。派 Explore agent 核对代码后修正旧脚本多处硬错误：频率选项 **10/15/30/60 分钟**（旧写 1/3/5/10/30）、**只有一个「自动监听备份」开关**统一 500ms 防抖（旧写"标签关闭后/窗口关闭后/空闲时"四开关是已废弃方案）、手动备份按钮文案 **「手动备份」/「确认备份」**（旧写"立即备份"）、导出分**概览页导出当前/列表行导出快照**两入口、导入/还原**真的 `chrome.tabs.create` 打开标签**非只存快照。明日 prod build 实机核对后开录。详见 dev-log 2026-08-17
- **教程视频封页/内容页/结尾页 PPT 提示词模板**（2026-08-15）：`docs/video-cover-prompt.md`。3 页统一模板（封页/本期内容/结尾页），后续每集只改字复用，整套 13 集视觉统一。风格简约大气科技（深墨蓝底 #0B1220 + 电光青蓝 #3B82F6 + 思源黑体 Heavy + 细线几何辉光）。含通用风格规范 + 带 `【】` 占位符的 3 页模板提示词 + EP01/EP02 填字版 + 封页/结尾页单独提示词 + 每集复用流程。素材以 video-script.md 实测为准（官网 www.ouu365.com / 署名 xpd）。EP01 已录完，明天生成 PPT 套进视频看效果，确认后批量生成 EP02~EP13
- **Bilibili 官方教程脚本**（2026-08-15）：`docs/video-script.md`（1080 行，13 集教程系列）。教程主线+场景演示，独立开发者本人讲课口吻；按学习曲线排序（备份提前到 E02 建立安全感）；每集统一结构（固定开场→第一人称痛点→操作→示例→固定结尾→片尾 0X/13）。**关键文案以 v1.4.1 代码实测为准**：E10 状态徽章 6 种（`lib/statusConfig.ts`：播放中/已静音/录制中/共享中/连接设备/引起注意）、E11 快捷键分平台（Win=Alt+Shift+1~4，Mac=Option+Shift+1~4，`lib/platform.ts`）、E02 换电脑不暗示云同步、E05 树形按打开关系显示父子层级、E08 稍后处理 vs 最近关闭分两个心智。13 集是教程基础设施，原始素材剪短视频多渠道分发。EP01 已录完待上传，待用户实机验证后继续批量录。详见 [[project-video-tutorial-script]]
...
### 待办（未完成）
- **🔥 Bilibili 教程视频录制（2026-08-17 E02 脚本已重写待录）**：`docs/video-script.md` 13 集脚本已定稿（E02 已按新结构重写 + v1.4.1 实测校正，详见 dev-log 2026-08-17），`docs/video-cover-prompt.md` 3 页 PPT 提示词模板已就绪。**EP01 已录完待上传**。**明天计划：①prod build 实机核对 E02 文案（自动监听首次告知框/还原下拉三选项/导入预览字段/四档频率）→ ②录 E02 → ③看回放验三问 → ④继续 E03**。整体策略：先看 EP01+E02 回放（像不像真人讲 / 新手能不能照做 / 有没有 PM 自嗨词），确认 OK 后批量录 E03–E13。**开录前必须用 prod build 实机核对**：①E10 六个状态徽章是否都能真实场景亮起 ②E11 两套快捷键（Win Alt+Shift+1~4 / Mac Option+Shift+1~4）是否真能全局触发 ③E02 备份导入导出全链路。录制素材后续剪短视频多渠道分发（B站/小红书/抖音/朋友圈/公众号）
