...
### 现状问题
- 当前：垃圾桶图标（`Trash2`）+ 文字「清理」，无下拉箭头
- 痛点：用户以为点一下就立刻批量删除，不敢点；不知道里面是一组可选的整理工具

### 设计结论

#### 1. 按钮文字
- **推荐：「整理」** —— 比「清理」更中性，传达「整理归类」而非「删除」
- **备选：「标签整理」** —— 更明确，但稍长（工具栏空间可能紧张）
- **备选：「清理工具」** —— 强调这是「工具集合」

#### 2. 图标
- **推荐：`ListChecks`** —— 清单+勾选，传达「先看清单再决定」的整理心智，而非「一键删除」
- **理由**：
  - `ListChecks` = 清单勾选，暗示「有条理地整理」
  - 比垃圾桶（`Trash2`）少很多攻击性，降低恐惧
  - 符合「检测后选择」的核心体验

#### 3. 下拉提示
- **必须加 `▾` 下拉箭头**，放在文字右侧
- 样式参考视图/排序按钮：`[图标] 整理 ▾`
- 目的：明确传达「点击展开菜单」，而非「点击执行操作」

#### 4. 菜单内部可感知优化
- **分组标题保持**：`直接关闭` / `检测后选择`（已在 PRD 3.1 中）
- **「直接关闭」组**：
  - 每个项加红色警告图标 `CircleX`（lucide）或用 `text-red-600` 强调
  - 文案明确动作后果，如「关闭非固定标签（将关 N 个）」——但 N 需要动态计算，MVP 可先不加
- **「检测后选择」组**：
  - 每个项加绿色安全图标 `Search` / `Clock`（已有）
  - 可加小字提示「先预览不立即关」（但菜单空间有限，可放 tooltip）

#### 5. 可感知原则对照（行为准则 §6）

| 阶段 | 设计 |
|------|------|
| **事前（预期）** | `ListChecks` 图标 +「整理」文字 + `▾` 箭头 → 用户预判：这是一个整理工具菜单，点击会展开选项，不会立刻删标签 |
| **事中（反馈）** | Hover 按钮高亮 → 点击弹出下拉菜单 → 菜单项分组清晰，危险项红色/安全项中性 |
| **事后（结果）** | 选择某项后：<br>- 「直接关闭」→ 弹 ConfirmDialog（数量/撤销提示）<br>- 「检测后选择」→ 弹 DetectReviewDialog（预览清单）<br>- 操作完成 → Toast 反馈（已关 N 个，Ctrl+Shift+T 恢复） |

### 最终按钮样式
```
[✓ 整理 ▾]
```
- 图标：`ListChecks`（lucide）
- 文字：「整理」
- 箭头：`ChevronDown`（lucide，尺寸 10，灰色）

### 修改文件
- `components/AppToolbar.vue`：按钮图标、文字、加箭头
