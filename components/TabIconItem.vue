<template>
  ...
  />
    ...
    <div class="absolute top-0.5 right-0.5 flex items-center gap-0.5">
      ...
      <button :class="['p-1 rounded', popover.isOpen(hoverCardId) ? 'bg-blue-100 dark:bg-blue-900/30 text-blue-600' : 'text-gray-500 hover:text-blue-600 hover:bg-gray-100 dark:hover:bg-gray-700']" @click.stop="onMenuClick" :title="t('action.moreActions')"><Menu :size="14" :stroke-width="2.25" /></button>
      <button class="p-1 text-gray-500 hover:text-red-500 hover:bg-red-50 dark:hover:bg-red-900/30 rounded" @click.stop="emit('close')" :title="t('common.close')"><X :size="14" :stroke-width="2.5" /></button>
    </div>
