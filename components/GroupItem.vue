<template>
  <div class="mb-2 border border-gray-200 rounded-lg overflow-hidden">
    ...
    <div class="flex items-center gap-2 px-3 py-2 bg-gray-50 hover:bg-gray-100 cursor-pointer" @click="toggleCollapse">
      ...
      <span class="flex-1 text-sm font-medium text-gray-900">{{ group.title || t('common.untitledGroup') }}</span>
      ...
      <button
        ...
        :title="tWithParams('group.item.dropInTitle', { count: dropInCount })"
      ...
      <div>
        ...
        <Teleport to="body">
          <div
            ...
            @click.stop>
            <button :class="btn" @click="startRename"><Edit :size="12" />{{ t('common.rename') }}</button>
            <div class="relative group/colorsubmenu">
              <button :class="btn" @click="">
                <Palette :size="12" />{{ t('group.item.changeColor') }}
            ...
            <button :class="btn" @click="onToggleCollapseFromMenu">
              ...
              {{ collapsed ? t('group.item.expand') : t('group.item.collapse') }}
            </button>
            <hr class="my-1 border-gray-100 dark:border-gray-700" />
            <button :class="[btn, 'text-red-600 hover:!bg-red-50 dark:hover:!bg-red-900/30']" @click="confirmUngroup"><FolderMinus :size="12" />{{ t('group.item.ungroup') }}</button>
            <button :class="[btn, 'text-red-600 hover:!bg-red-50 dark:hover:!bg-red-900/30']" @click="confirmCloseAll"><X :size="12" />{{ t('group.item.closeAll') }}</button>
          </div>
        </Teleport>
      </div>
    </div>
    ...
    <Teleport to="body">
      <div v-if="showRenameDialog" class="fixed inset-0 z-[100] flex items-center justify-center">
        ...
        <div class="relative bg-white dark:bg-gray-800 rounded-lg shadow-xl w-72 p-4">
          <h3 class="text-sm font-semibold text-gray-900 dark:text-gray-100 mb-3">{{ t('group.item.rename.title') }}</h3>
          ...
          <div class="flex justify-end gap-2">
            <button class="px-3 py-1.5 text-xs text-gray-700 dark:text-gray-200 hover:bg-gray-100 dark:hover:bg-gray-700 rounded-md" @click="showRenameDialog = false">{{ t('common.cancel') }}</button>
            <button class="px-3 py-1.5 text-xs bg-blue-600 text-white rounded-md hover:bg-blue-700" @click="finishRename">{{ t('common.confirm') }}</button>
          </div>
        </div>
      </div>
    </Teleport>
    ...
    <Teleport to="body">
      <div v-if="showConfirmDialog" class="fixed inset-0 z-[100] flex items-center justify-center">
        ...
        <div class="relative bg-white dark:bg-gray-800 rounded-lg shadow-xl w-72 p-4">
          ...
          <div class="flex justify-end gap-2">
            <button class="px-3 py-1.5 text-xs text-gray-700 dark:text-gray-200 hover:bg-gray-100 dark:hover:bg-gray-700 rounded-md" @click="showConfirmDialog = false">{{ t('common.cancel') }}</button>
            <button class="px-3 py-1.5 text-xs bg-red-600 text-white rounded-md hover:bg-red-700" @click="handleConfirm">{{ t('common.confirm') }}</button>
          </div>
        </div>
      </div>
    </Teleport>
...
const confirmUngroup = () => {
  popover.close(menuPopoverId.value)
  confirmTitle.value = t("group.item.ungroup")
  confirmMessage.value = t("group.item.confirm.ungroup.msg")
...
const confirmCloseAll = () => {
  popover.close(menuPopoverId.value)
  confirmTitle.value = t("group.item.closeAll")
  confirmMessage.value = tWithParams("group.item.confirm.closeAll.msg", { count: props.tabs.length })
