<template>
  ...
  <div>
    ...
    <div class="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg p-5 mb-4">
      <div class="flex items-start gap-3">
        <div class="flex-1 min-w-0">
          <p class="text-xs text-gray-500 dark:text-gray-300 uppercase tracking-wide mb-1">最新快照</p>
    ...
    <!-- 【次】统计行 -->
    <div class="flex items-center gap-2 text-[11px] text-gray-500 dark:text-gray-300 mb-3 px-1 flex-wrap">
      ...
      <span class="ml-auto text-gray-500 dark:text-gray-300">上限 {{ settings.cacheMaxSnapshots }} 个 / {{ fmtBytes(settings.cacheQuotaBytes) }}</span>
    </div>
    ...
    <div class="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg overflow-hidden">
      <div v-if="!snapshots.length" class="px-5 py-12 text-center text-xs text-gray-500 dark:text-gray-300">
        ...
        >
          ...
          <div class="flex-1 min-w-0">
            <p class="font-medium text-gray-800 dark:text-gray-100">
              {{ fmtFull(s.createdAt) }}
              <span class="ml-1 text-xs text-gray-500 dark:text-gray-300">来源 {{ sourceLabel(s.source) }}</span>
              <span v-if="s.locked" class="ml-1 text-xs text-amber-600">已锁定</span>
              <span v-if="s.label" class="ml-1 text-xs text-blue-500">「{{ s.label }}」</span>
            </p>
          ...
          <div class="shrink-0 flex items-center gap-1.5">
            <button
              class="text-[11px] px-2.5 py-1.5 min-h-[32px] rounded border border-gray-200 dark:border-gray-600 text-gray-500 hover:bg-gray-50 dark:hover:bg-gray-700"
            ...
            >导出</button>
            <button
              class="text-[11px] px-2.5 py-1.5 min-h-[32px] rounded border border-amber-200 text-amber-600 hover:bg-amber-50 dark:border-amber-800 dark:hover:bg-amber-900/30"
              @click="$emit('toggleLock', s.id, !s.locked)"
            >{{ s.locked ? '解锁' : '锁定' }}</button>
            <button
              class="text-[11px] px-2.5 py-1.5 min-h-[32px] rounded border border-red-200 text-red-500 hover:bg-red-50 dark:border-red-800 dark:hover:bg-red-900/30"
