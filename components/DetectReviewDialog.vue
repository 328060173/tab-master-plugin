      ...
      >
        ...
        <div class="px-5 pt-4 pb-3 border-b border-gray-100 dark:border-gray-700">
          ...
          <div v-if="mode === 'unused'" class="flex items-center gap-1.5 mt-2 flex-wrap">
            <label class="text-[11px] text-gray-600 dark:text-gray-300">{{ t('dialog.detectReview.threshold') }}</label>
            <button
            ...
            <input
              ...
              :title="t('dialog.detectReview.customDaysTitle')"
              @change="onCustomDaysInput"
            />
            <span class="text-[10px] text-gray-400">{{ t('dialog.detectReview.days') }}</span>
            <span class="text-[10px] text-gray-400 ml-auto">{{ t('cleanup.detect.excludeNote') }}</span>
          </div>
        </div>
        ...
        <div class="px-5 py-2 border-b border-gray-100 dark:border-gray-700 flex items-center justify-between">
          <button
            ...
            @click="toggleAll"
          >{{ allSelected ? t('cleanup.detect.unselectAll') : t('cleanup.detect.selectAll') }}</button>
          <span class="text-[11px] text-gray-500 dark:text-gray-400">{{ tWithParams('dialog.detectReview.selected', { selected: selectedCount, total: totalCount }) }}</span>
        </div>
        ...
        <div class="flex-1 overflow-y-auto px-3 py-2">
          ...
          <div v-if="totalCount === 0" class="text-center py-8 text-xs text-gray-400">
            {{ mode === 'duplicates' ? t('cleanup.toast.noDuplicates') : tWithParams('dialog.detectReview.noUnused', { days: customDays }) }}
          </div>
          ...
          <template v-else-if="mode === 'duplicates'">
            <div v-for="(g, gi) in groups" :key="g.url" :class="['mb-3', gi > 0 ? 'pt-3 border-t border-gray-100 dark:border-gray-700' : '']">
              <p class="text-[10px] text-gray-400 truncate mb-1 px-1" :title="g.url">{{ g.url }} {{ tWithParams('dialog.detectReview.groupCount', { count: g.items.length }) }}</p>
              <div v-for="(item, idx) in g.items" :key="item.id" class="flex items-center gap-2 px-2 py-1.5 rounded hover:bg-gray-50 dark:hover:bg-gray-700">
                ...
                <!-- 每组首条（最新）标"建议保留" -->
                <span v-if="idx === 0" class="text-[9px] text-green-600 bg-green-50 dark:bg-green-900/30 px-1.5 py-0.5 rounded shrink-0">{{ t('cleanup.detect.suggestKeep') }}</span>
              </div>
            </div>
          </template>
        ...
        <div class="px-5 py-3 border-t border-gray-100 dark:border-gray-700 flex gap-2 justify-end items-center">
          ...
          <button
            ...
            @click="confirmClose"
          >{{ tWithParams('cleanup.detect.closeButton', { count: selectedCount }) }}</button>
        </div>
      </div>
    </div>
  </Teleport>
...
const title = computed(() =>
  props.mode === "duplicates"
    ? tWithParams('cleanup.detect.duplicatesTitle', { groups: props.groups?.length || 0, total: totalCount.value })
    : totalCount.value === 0
      ? tWithParams('dialog.detectReview.noUnused', { days: customDays.value })
      : tWithParams('cleanup.detect.unusedTitle', { total: totalCount.value }),
)
const summary = computed(() =>
  props.mode === "duplicates"
    ? t('cleanup.detect.duplicatesSummary')
    : t('cleanup.detect.unusedSummary'),
)

// 列表单条"上次访问"标签的文案
const accessLabelFor = (item: TabItem): string => {
  const at = getEffectiveAccessTime(item)
  if (at === undefined) return t('cleanup.detect.accessUnknown')
  const diff = Date.now() - at
  if (diff < 60_000) return t('cleanup.detect.accessRecent')
  return tWithParams('cleanup.detect.notVisitedFor', { duration: formatUnusedDuration(diff) })
}
</script>
