/**
 * 紧凑版 MD5 实现（RFC 1321），用于头像本体按邮箱 hash 取色。
 * 公共领域实现（blueimp/JavaScript-MD5 简化版）。
 *
 * 为什么不用 Web Crypto：Web Crypto 不提供 MD5（已弃用）。
 * 这里 MD5 仅用作"同一邮箱 → 同一颜色"的稳定 hash，不用于安全场景，
 * 用 MD5 是为了和 PRD avatar-theme-visual §1.1 对齐（未来若接 Gravatar 兼容也同算法）。
 */
...
function strToBins(str: string): number[] {
  const utf8 = unescape(encodeURIComponent(str))
  const blockCount = ((utf8.length + 8) >> 6) + 1
  const bins = new Array(blockCount * 16).fill(0)
  for (let i = 0; i < utf8.length; i++) {
    bins[i >> 2] |= utf8.charCodeAt(i) << ((i % 4) * 8)
  }
  bins[utf8.length >> 2] |= 0x80 << ((utf8.length % 4) * 8)
  bins[blockCount * 16 - 2] = utf8.length << 3
  return bins
}

function binlToHex(binarray: number[]): string {
